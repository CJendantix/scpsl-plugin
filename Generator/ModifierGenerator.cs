...
public sealed class ModifierGenerator : IIncrementalGenerator
{
        ...
        (Compilation, ImmutableArray<ClassDeclarationSyntax>) data)
    {
        ...
        rootCfg.AppendLine("/// <summary>Configuration class generated for all modifiers.</summary>");
        rootCfg.AppendLine("public sealed class Configuration");
        rootCfg.AppendLine("{");
        ...
        foreach (ClassDeclarationSyntax cls in classes)
        {
            ...
            instanceGen.AppendLine($"/// <summary>Partial class to add Instance property to {sym.Name}.</summary>");
            instanceGen.AppendLine($"partial class {sym.Name}");
            instanceGen.AppendLine("{");
            instanceGen.AppendLine($"    /// <summary>Singleton instance of {sym.Name}.</summary>");
            instanceGen.AppendLine($"    public static {sym.Name} Instance {{ get; }} = new {sym.Name}();");
            instanceGen.AppendLine("}");
            context.AddSource($"{sym.Name}.Instance.g.cs", instanceGen.ToString());

            // Add to loader
            loader.AppendLine($"        _ = {fullTypeName}.Instance;");
